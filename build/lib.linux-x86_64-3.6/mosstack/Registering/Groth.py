...
class Groth(Registering):
    """
    Stellar matching implementation which uses SExtractor to find stars and method by E.J. Groth to do the star
    matching itself.
    ...
    def step1(frame):
        ...
        newtri = []
        for tri in frame.tri:      # tri is a list of coordinates. ((x1,y1),(x2,y2),(x3,y3))
            ...
            temp.append(tC)
            newtri.append(temp)
        frame.tri = np.float32(np.array(newtri))
        frame.step1 = True

        print("Step 1 complete for image " + str(frame.number) + ". " + str(len(frame.tri)) + " triangles found.")
    ...
    def step2(f1, f2):
        ...
        print("Matching image " + str(f1.number) + " to reference image " + str(f2.number) + "...")

        f1.pairs = _step2(f2.tri, f1.tri)
        f1.step2 = True
